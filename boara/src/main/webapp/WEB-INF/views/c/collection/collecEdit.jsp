...
<c:if test="${not empty SID}">
	...
	<div class="w3-row-padding w3-padding-64 w3-container">
		<%-- 데이터 비교용 --%>
		<input type="hidden" id="tcname" value="${CINFO.cname}">
		<input type="hidden" id="tdescr" value="${CINFO.descr}">
		<input type="hidden" id="tgenre" value="${CINFO.sgenre}">
		<input type="hidden" id="tfno" value="${CINFO.fno}">
	
		<div class="w3-col w3-padding">
			<form method="POST" name="frm" id="frm" class="w3-center" encType="multipart/form-data"
				action="/boa/collection/collecEditProc.boa">
...
<c:if test="${CINFO.sgenre eq 'empty'}">
	<c:forEach var="genr" items="${GLIST}">
						<label class="w3-col m3 left"><input type="checkbox" id="genr" name="genr" value="${genr.gno}"> ${genr.gname}</label>
	</c:forEach>
</c:if>
...
<c:if test="${not empty HISTORY}">
				<div class="w3-col w3-margin-bottom">
					<label class="w3-col m3 w3-padding right">썸네일 히스토리 중 선택 : </label>
